...
| **包管理器** | npm |
| **当前版本** | 0.1.0 |
| **当前完成度** | 约 95%——P0/P1/P2 全部完成，20 个配置模块编辑器全部实现，Zod 运行时校验已集成，YAML diff 已实现，138 个测试全部通过（含 2026-06-14 审查回归测试 + 2026-06-18 链路构建器修复） |
...
| **当前分支** | `main` |
| **当前 commit** | `67fd40b` |
| **HEAD 对应** | `merge: reverse chain-builder display order` |
| **未提交改动** | 仅本文档更新；另有未跟踪审查文档 `docs/CODE_REVIEW_REPORT_2026-06-14.md`、`docs/PROJECT_REVIEW_REPORT.md` |
| **dev/* 分支** | 57 个（见下方，含已合并历史） |
...
dev/fix-undo-redo-history
dev/fix-search-index-mismatch
dev/fix-rule-reference-parser
dev/fix-export-copy-bypass
dev/fix-dialer-proxy-cycle-detection
dev/fix-stash-dns-respect-rules
dev/fix-url-import-cors-proxy
dev/add-navtree-search-filter
dev/fix-topology-config-sync
dev/fix-yaml-internal-fields-leak
dev/fix-autosave-blank-draft
dev/fix-sidebar-responsive-inline
dev/fix-topology-white-screen
dev/fix-topology-key-remount
dev/reverse-chain-display-order
dev/render-complete-dialer-chains
dev/improve-editor-layouts
```

### 最近合并到 main 的 commit（按时间倒序）

```
67fd40b merge: reverse chain-builder display order
9536b0d fix: reverse chain-builder display order to match data transmission direction
be2bfd1 merge: render complete dialer proxy chains
a166637 fix: render complete dialer proxy chains
6b9066b merge: improve editor layouts and rule workflows
f522562 fix: improve editor layouts and rule workflows
b7c85eb merge: fix React Flow white screen with controlled mode and topology tests
eb7d302 fix: use controlled ReactFlow mode to avoid race condition, add topology component tests
8bce6e6 merge: fix React Flow topology white screen with functional node/edge merge
f353ca4 merge: fix sidebar inline width overriding responsive class on narrow viewports
e394606 fix: skip inline sidebar width on narrow viewports to allow Tailwind max-md:w-0 to take effect
986adfe merge: prevent blank auto-save draft on first app open
abc524c merge: strip internal _comments field from stringifyYaml output
ac8e93a fix: strip _comments from stringifyYaml output to prevent internal field leak
ecc28bf merge: sync React Flow topology on config change
4256dbb fix: sync React Flow topology nodes/edges when config changes
3e65cf6 merge: add NavTree search filtering
2a7ed88 feat: add search filtering to NavTree with test coverage
4b99573 merge: sync URL import CORS proxy with settings page
0b7db06 fix: read CORS proxy URL from localStorage on init and sync back on change
18b4fcc merge: fix Stash DNS respect-rules conversion and sub-rules unreachable error
8cc3719 fix: delete respect-rules after converting to follow-rule, move sub-rules error before removal loop
f2f271d merge: add dialer-proxy multi-node circular chain detection
19afc1d fix: add DFS cycle detection for dialer-proxy chains
a9e290a merge: apply canExport check consistently to copy and download
f692fa4 fix: apply canExport check to copy-to-clipboard button as well
2ffa991 merge: fix rule reference target parsing and MATCH UI display
00cdb70 fix: add shared rule parser to fix no-resolve target misidentification and MATCH UI parsing
40f066e merge: preserve original index in filtered proxy/group search results
922c6ff fix: preserve original index in filtered search results to prevent editing wrong items
22592f4 merge: fix undo/redo history to save snapshot after config update
20899d0 fix: save history snapshot after config update to fix undo/redo round-trip
...
| 组件渲染测试 | 已完成 | `src/__tests__/components/` | YamlDiff / GeneralEditor / NavTree / ExportDialog / ProxyGroupTopology 5 个 smoke test |
...
- **左侧 NavTree**: 20 个配置模块入口，支持搜索过滤（含父子节点匹配），有组件测试覆盖
- **底部固定链接**: 设置 + About
...
> **所有命令均在修复分支提交 `9dbb2f1` 合并到 `main` 前执行，修复已通过 merge commit `4accbc5` 合并到 `main`，时间：2026-06-14。**
...
| `npm run build` (`tsc -b && vite build`) | ✅ 通过 | 已按需加载/分包：入口 JS 73.76KB gzip 20.42KB；最大 vendor chunk 为 CodeMirror 358.37KB gzip 114.72KB |
| `npm test` (`vitest run`) | ✅ **138 passed** | 25 个测试文件全部通过 |
...
| `src/__tests__/rule-validator.test.ts` | 8 | 不可达规则/缺失 MATCH/重复规则/规则格式错误 |
...
| `src/__tests__/config-store.test.ts` | 8 | updateConfig/undo/redo/setConfig history 隔离/reset/setName |
...
| `src/__tests__/components/NavTree.test.tsx` | 6 | NavTree 导航项渲染/搜索输入/过滤结果验证 |
...
| `src/__tests__/components/RulesEditor.test.tsx` | 3 | 规则搜索过滤/拖拽禁用/按原始 index 删除 |
...
| README / 交接文档口径过期 | P3 | 同步为 120 tests / 22 test files、20 个配置模块、当前工作区状态 |
...
> P0/P1 已全部完成。`PROJECT_REVIEW_REPORT.md` 和 `CODE_REVIEW_REPORT_2026-06-14.md` 中列出的 P1/P2/P3 修复均已处理；以下为剩余体验增强或长期优化建议。
...
> **文档生成时间**: 2026-06-14（更新）
> **文档作者**: AI Agent（基于仓库实际状态生成）
> **仓库状态**: `main` 分支 `4accbc5`，本轮审查修复已合并；当前仅本文档更新尚未提交，另有两个未跟踪审查文档；120/120 测试通过，0 ESLint warnings，构建已拆分入口和重依赖 chunk
