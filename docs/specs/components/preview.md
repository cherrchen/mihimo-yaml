...
| 组件 | 说明 |
|------|------|
| `YamlPreview` | 右侧预览面板：标签页切换 YAML 语法高亮 / 完整性问题 / 差异对比 / 兼容性占位四个视图 |
| `YamlDiff` | YAML 差异比对组件：行级 diff 展示，绿色标记新增、红色标记删除，底部统计变更行数 |

## UI 结构
`YamlPreview` 占据侧边面板全高，采用 flex 纵向布局：
- **标签栏**：四个标签页 — "YAML"（语法高亮的只读 CodeMirror 编辑器）、"问题"（展示 `integrityReport` 中的 error/warning 条目，有计数徽章）、"对比"（调用 `YamlDiff` 对比上次和当前 YAML）、"兼容性"（占位提示）
- **内容区**：根据 `previewMode` 切换渲染对应视图
- **底部状态栏**：显示当前 YAML 的行数和字符数
...
## 依赖
- **状态管理**: `useConfigStore` (config, integrityReport), `useUiStore` (previewMode, setPreviewMode, theme)
- **工具函数**: `stringifyYamlOrdered` from `@/schema/yaml`
- **第三方库**: `@uiw/react-codemirror` (CodeMirror 编辑器), `@codemirror/lang-yaml` (YAML 语法), `@codemirror/theme-one-dark` (暗色主题), `diff` (diffLines)
