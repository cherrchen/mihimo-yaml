...
创建、导入、编辑、校验并导出 mihomo 和 Stash 兼容 YAML 配置文件。19 个配置模块编辑器，Zod 运行时校验，YAML diff 对比，92 个测试全部通过。

## 功能

- 从 YAML 文件、URL、剪贴板导入配置（URL 拉取支持自定义 User-Agent、CORS 代理）
- Zod 运行时 schema 校验，导入时逐字段类型错误检测
- 图形化编辑 19 个 mihomo 配置模块（General、DNS、Proxies、Proxy Groups、Rules、iptables、ebpf 等）
...
```
92 tests · 18 test files · 0 ESLint warnings
```
